...
本次数值稳定性修复后，实验 12 的 checkpoint 版本为 **7**。旧版本 6 的实验 12 checkpoint 由零残差初始化处的非有限梯度产生，训练结果无效；训练恢复与推理加载都会因版本不匹配而拒绝该 checkpoint，必须从头训练。
...
这限制的是经过 `tanh(α)` 和可靠性门控后的**最终有效残差**，而非仅限制网络内部未调制的 `ΔF`。当残差本来较小时，`s=1`，不改变其大小；只有超过上限时才按比例缩小。稳定项 `10^-12` 位于平方根内部：初始 `α=0`、`C=0` 时，`r_C=10^-6` 且反向梯度保持有限；不能将稳定项放在 `sqrt(mean(C²))` 之后，否则零点反向会产生 NaN 并使 AMP 跳过整个生成器更新。
